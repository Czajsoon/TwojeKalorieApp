...
void ustawProdukt(posilek** PosilekHead, posilek* akktualnyPosilek, const std::string& liniaProduktu);
...
void czytaj(posilek** PosilekHead) {
    ...
    while (std::getline(file, linia)) {
        ...
        if (typ_lini == 2 && nowyposilek != nullptr && numer_lini > 0) {
            ustawProdukt(PosilekHead, nowyposilek, linia);
        }
...
void ustawProdukt(posilek** PosilekHead, posilek* akktualnyPosilek, const std::string& liniaProduktu) {
    ...
    addProductToMeal(PosilekHead, akktualnyPosilek->nazwa_posilku, nazwaPro, WE, waga);
}
